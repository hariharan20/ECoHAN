...
parser.add_argument('scene_type' , type=str , help = 'Choose one from "open_space" ,  "corridor"  , "long_bottleneck" ,  "short_bottleneck" ')

args = parser.parse_args()
ros_pack = rospkg.RosPack()
backoff_file_path  = ros_pack.get_path('ecohan')  + '/config/conversation.yaml'
...
class scene_gen: 
    def __init__ (self):
        ...
        rospy.set_param('task_started' , False)
        rospy.set_param('task_done' ,False)
        self.human_id = 2
    ...
    def backoff_cb(self , _) :
        ...
        if rospy.get_param('backoff/human' , False)  :
    ...
    def eval_scene_generator(self , setting ): 
        ...
        rospy.set_param('task_started' , True)
        self.robot_mover()
        self.human_mover()
...
if __name__ == "__main__" :
    ...
    obj = scene_gen()
    obj.eval_scene_generator(args.scene_type)
