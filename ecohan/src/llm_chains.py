...
model = OllamaLLM(model=rospy.get_param('llm_model' , default ='llama3.2') , 
                  ...
                  base_url = rospy.get_param('llm_base_url'))
...
convo_prompt = None
convo_output = None
convo_output_parser = JsonOutputParser(pydantic_object=robot_convo_output)
convo_chain = convo_prompt | model | convo_output_parser

def convo(input) : 
    response = convo_chain.invoke({
        'dialogue_history' : input,
        'output_format' : convo_output
    })
    return response
